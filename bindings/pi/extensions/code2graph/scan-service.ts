...
import { code2graph } from "./code2graph-node.ts";
...
export async function collectSources(root: string, hidden: boolean, max: number, signal?: AbortSignal, supports: (file: string) => boolean = file => Boolean(code2graph.languageOf(file))) {
...
function worker(files: Array<{ file: string; source: string }>, tier: Tier, signal: AbortSignal): Promise<{ facts: FileFacts[]; graph: CodeGraph }> {
...
function createOperation(key: string, root: string, tier: Tier, maxFiles: number, hidden: boolean, refresh: boolean): Operation {
  ...
  const promise = (async () => {
    ...
    active++;
    try {
      const native = await worker(collected.out, tier, controller.signal); stopped(controller.signal);
      const result: ScanResult = { root, tier, files: native.facts, graph: native.graph, warnings: collected.warnings, truncated: collected.truncated, totalBytes: collected.bytes, cache: { hit: false, ageMs: 0 }, indexes: buildIndexes(native.graph) };
...
export function configureScanForTests(options: { timeoutMs?: number; workerDelayMs?: number }) { if (options.timeoutMs !== undefined) maxMs = options.timeoutMs; if (options.workerDelayMs !== undefined) workerDelayForTests = options.workerDelayMs; }
export function resetScanTestConfiguration() { maxMs = 30_000; workerDelayForTests = 0; }
