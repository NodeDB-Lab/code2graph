...
class WorkflowContractTests(unittest.TestCase):
    ...
    def test_distribution_provenance_source_and_no_build_contracts(self):
        ...
        self.assertEqual(github["environment"], "github")
        self.assertEqual(github["steps"][0]["uses"], "actions/checkout@v6")
        self.assertEqual(github["steps"][0]["with"]["ref"], "${{ inputs.tag }}")
        release_step = next(step for step in github["steps"] if step.get("name") == "Create or update exact GitHub release")
        self.assertEqual(release_step["env"]["GH_TOKEN"], "${{ secrets.GH_RELEASE_TOKEN || github.token }}")
